...
public final class TicketConversationDialog {
    ...
    public static Dialog create(
        ...
        final DialogActionCallback closeCallback
    ) {
        ...
        if (messages.isEmpty()) {
            ...
            for (final TicketMessageRecord message : messages) {
                final String authorType = message.staffMessage() ? "[Staff]" : "[Joueur]";
                body.add(DialogBody.plainMessage(
                    Component.text(DATE_FORMAT.format(message.timestamp()) + " " + authorType + " " + message.authorName() + " : " + message.content()),
                    360
                ));
            }
        }
        final List<ActionButton> actions = List.of(
            ...
            DialogHelper.button(
                Component.text("Fermer le Dialog"),
                180,
                DialogAction.customClick(closeCallback, DialogHelper.singleUseOptions())
            )
        );
        return DialogHelper.create(
            Component.text("Ticket #" + ticket.id() + " — " + ticket.targetName()),
            body,
            List.of(DialogInput.text("message", Component.text("Ton message"))
                .width(300)
                .labelVisible(true)
                .initial("")
                .maxLength(200)
