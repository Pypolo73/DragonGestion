...
public final class SanctionCommand implements CommandExecutor, TabCompleter {
    ...
    private void startInteractive(final Player player, final OfflinePlayer target, final SanctionType type, final boolean timed, final String label) {
        ...
        if (timed) {
            this.plugin.getDialogSupportService().openTimedReasonFlow(player, title, (reason, duration) -> execute(player, target, type, reason, duration));
        } else {
            this.plugin.getDialogSupportService().openReasonFlow(player, title, reason -> execute(player, target, type, reason, null));
        }
    }
